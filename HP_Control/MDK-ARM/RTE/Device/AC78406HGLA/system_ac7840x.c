...
void InitSram(void)
{
    ...
    data = 0U;
    if (0U == (SPM->STB_WP_STATUS & 0x1FFFFU))
    {
        addr = SRAM_L_START_ADDRESS;
        endAddr = SRAM_U_END_ADDRESS;
        /* Initialize the SRAM to 0 */
        while (addr < endAddr)
        {
    ...
    {
        addr = SRAM_L_START_ADDRESS;
        endAddr = SRAM_L_END_ADDRESS - SRAM_RETENTION_SIZE;
        /* Initialize the SRAM to 0 */
        while (addr < endAddr)
        {
            *(__IO uint32_t *)addr = data;
